...
int main(int argc, char **argv) {
	...
	fstream myfile;
	myfile.open("resultfileThreads.txt", std::ios::out);
	...
	for (timestep; timestep < n_steps; timestep += 8)
	{
		for (auto it = std::begin(threads); it != std::end(threads) - 1 && timestep < 8; ++it)
		{
			*it = thread(worker, m);
			timestep++;
		}
	...
	threads.back() = std::thread(worker, m);
	...
	// End timer for Mesh, Begin timer for PPM //
	...
	std::vector<std::thread> heatdata(8);
	for (int y = 0; y < h; y++)
	for (int x=0;x<w;x++) {
